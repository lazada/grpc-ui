...
function ServiceGrpcCtrl($scope, $http) {
    ...
    function invokeStream(data, onMessage, onError, onComplete) {
        var ws = new WebSocket('ws://' + location.host + '/invoke?stream=1');
        ws.onopen = function () {
            ...
            for (var s in services) {
                ...
                for (var m in service.methods) {
                    ...
                    grpc_descr[service.package_name][service.name][method.name] = {
                        output_errors: [],
                        full_name: service.package_name + '.' + service.name + '.' + method.name,
                        can_invoke: true,
                        is_stream: false,
                        loading: false,
                    };
    ...
    $scope.invoke_stream_method = function (method) {
            method.output_log = [];
            method.output_result_formatted = '';
            method.output_errors = [];
            method.invoking = true;

            method.cancel = invokeStream({
                    addr: $scope.addr,
                    grpc_method: method.full_name,
                    grpc_args: JSON.stringify(makeArgs(method)),
                },
                function (msg) {
                    method.output_log.unshift(JSON.stringify(JSON.parse(msg), null, 4));
    ...
    $scope.stop_stream_method = function (method) {
        method.cancel();
        method.cancel = null;
    };

    $scope.invoke_method = function (method) {
        method.loading = true;
        method.output_result_formatted = '';
        method.output_errors = [];
        invoke({
        ...
        .then(function (resp) {
            method.output_result_formatted = JSON.stringify(JSON.parse(resp.data), null, 4);
        })
        .catch(function (resp) {
            method.output_errors.push(resp.data.error);
        })
        .finally(function () {
